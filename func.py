...
def newProduct(user_id, product_type, quantity, product_description, address):
    product_id = random.randint(1000000000000000, 9999999999999999)

    mycursor = mydb.cursor()
    mycursor.execute("use covidApp")
    mycursor.execute(f"INSERT INTO sellerInfo "
                     f"(product_id, user_id, product_type, quantity, product_description, address) VALUES "
                     f"({product_id}, {user_id}, '{product_type}', {quantity}, '{product_description}', '{address}')")
    mydb.commit()
